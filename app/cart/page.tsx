      ...
      <div className="w-2/3 mx-auto">
        {cartItems.map((item: any) => (
          <div className="flex items-center justify-between mb-6 py-3 px-6 rounded bg-slate-50">
            ...
            <h5 className="w-1/6 font-medium">${item.price}</h5>
            <div className="w-1/4 flex gap-4">
              <button className="text-lg p-1 bg-slate-100 h-6 w-6 flex items-center justify-center border border-slate-200 cursor-pointer">+</button>
              <button className="text-lg p-1 bg-slate-100 h-6 w-6 flex items-center justify-center border border-slate-200 cursor-pointer">-</button>
            </div>
            <button className="text-red-500 hover:bg-red-100 p-2 rounded " onClick={() => handleRemove(item.id)}>
              <FaTrashAlt/>
            </button>
          </div>
        ))}
